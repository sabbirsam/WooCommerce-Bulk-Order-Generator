...
jQuery(document).ready(function ($) {
  function initializeDelete(type) {
    ...
    $.ajax({
      ...
      error: function error(jqXHR, textStatus, errorThrown) {
        handleError(type, "Failed to initialize: ".concat(errorThrown));
      }
    });
  }
  function processBatch(type, offset, total, skipped) {
    $.ajax({
      ...
      success: function success(response) {
        if (response.success) {
          var processed = offset + response.data.deleted;
          ...
          $("#".concat(type, "-processed")).text(processed);
          ...
          if (!response.data.done) {
            ...
            var successCount = processed - newSkipped;
            console.log("Deletion Summary:\n                            Total: ".concat(total, "\n                            Processed: ").concat(processed, "\n                            Successful: ").concat(successCount, "\n                            Skipped: ").concat(newSkipped));
            // Verify deletion
      ...
      error: function error(jqXHR, textStatus, errorThrown) {
        handleError(type, "Failed to process batch: ".concat(errorThrown));
      }
    });
  }
  function verifyDeletion(type, skipped) {
    $.ajax({
      ...
      success: function success(response) {
        if (response.success) {
          ...
          if (remainingCount > skipped) {
            handleError(type, "Unexpected number of ".concat(type, "s remained. Expected ").concat(skipped, ", found ").concat(remainingCount, ". Please try again."));
          } else {
            $("#delete-".concat(type, "s-btn .poc-spinner")).hide();
            // Updated success message to include skipped items
            alert("Operation completed!\n".concat(skipped, " ").concat(type, "s were skipped\n").concat(remainingCount === skipped ? 'All remaining' : 'Some', " ").concat(type, "s have been successfully deleted!"));
            location.reload();
          }
        } else {
      ...
      error: function error(jqXHR, textStatus, errorThrown) {
        handleError(type, "Failed to verify deletion: ".concat(errorThrown));
      }
    });
  }
